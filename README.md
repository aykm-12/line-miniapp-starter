...
3. ローカルで動かす
   ...
   **`npm run dev` でローカル起動し、PCブラウザで確認するのが基本的な開発方法です。** デプロイ後のURLは下記の仕組みによりLINEアプリ上でしか正常に開けなくなるため、コードの動作確認はまずローカル環境で行ってください。
...
- Supabaseの無料プランは、過去7日間データベースへのアクセスがないとプロジェクトが自動的に一時停止（Pause）されます。放置後に読み書きが失敗する場合はこれが原因の可能性が高いです。再開方法は [docs/02_supabase_setup.md](docs/02_supabase_setup.md#2-4-プロジェクトの一時停止pauseと再開) を参照してください。
